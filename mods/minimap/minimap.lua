...
DF:NewDefaults('minimap', {
    ...
    minimapAnimation = {value = false, metadata = {element = 'checkbox', category = 'Animation', indexInCategory = 1, description = 'Enable minimap animations', dependency = {key = 'showMinimap', state = true}}},
    ...
    minimapAnimation2 = {value = false, metadata = {element = 'checkbox', category = 'Animation', indexInCategory = 8, description = 'Enable second animation layer', dependency = {key = 'showMinimap', state = true}}},
...
DF:NewModule('minimap', 1, 'PLAYER_LOGIN', function()
    ...
    customArrowFrame, customArrowTex, originalArrow = DF.lib.CreateCustomPlayerArrow()
    local callbacks = {}
                ...
                pingFrame:SetScript('OnEvent', function()
                    if arg1 and arg2 and arg3 then
                        ...
                        if name then
                            ...
                            pingTimerId = DF.timers.delay(3, function()
                                pingText:Hide()
                                pingTimerId = nil
                            end)

                            if DF.profile.minimap.printPing then
    ...
    callbacks.timeFormat = function(value)
        DF.date.timeFormat = value
    end

    callbacks.showSeconds = function(value)
        DF.date.showSeconds = value
    end

    callbacks.amPmColor = function(value)
        DF.date.amPmColor = value
    end

    DF:NewCallbacks('minimap', callbacks)
end)
